...
// Endpoint D: GET /api/analysis/workload-distribution
router.get('/workload-distribution', async (req, res) => {
    try {
        const facultyList = await Faculty.find().lean();
        const workloadData = [];

        // This could be optimized with aggregation pipeline
        for (const faculty of facultyList) {
            const count = await Timetable.countDocuments({ facultyId: faculty._id });
            workloadData.push({
                name: faculty.name,
                classes: count,
...
// Endpoint E: GET /api/analysis/conflict-heatmap
router.get('/conflict-heatmap', async (req, res) => {
    try {
        // Simple heatmap: Count of classes per day/time
        // Assuming Timetable has day and time (or linked TimeSlot)
        // If Timetable uses TimeSlot, we need to populate.

        const sessions = await Timetable.find().populate('timeslotId').lean();

        // Structure: { "Monday": { "09:00-10:00": 5, ... }, ... }
        const heatmap = {};

        sessions.forEach(session => {
            if (session.timeslotId) {
                const day = session.timeslotId.day; // e.g., 'Monday'
                const time = `${session.timeslotId.startTime}-${session.timeslotId.endTime}`;

                if (!heatmap[day]) heatmap[day] = {};
                if (!heatmap[day][time]) heatmap[day][time] = 0;
                heatmap[day][time]++;
            }
        });
